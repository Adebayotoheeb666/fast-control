                ...
                <div className="form-group">
                    ...
                    <select {...props}>
                        {this.props.data?.options?.length > 0
                            ? this.props.data.options.map((option, ix) => {
                                  const this_key = `preview_${option?.key ?? ix}`;
                                  return (
                                      <option value={option?.value ?? null} key={this_key}>
                                          {option?.text ?? ''}
                                      </option>
                                  );
                              })
...
class Tags extends React.Component {
    ...
    render() {
        ...
        if (!this.props.mutable) {
            props.value = options?.length > 0 ? options[0]?.text : null;
